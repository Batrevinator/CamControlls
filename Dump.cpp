...
int main(int argc, char** argv)
{
    ...
    const double MAX_AREA = 9000.0;
    const double MIN_AREA = 3000.0;
    cv::Point lastHandPos(-1, -1);
    const double DIST_WEIGHT = 2.0; // Tune this value for how much to prefer proximity
    ...
    while (true) {
        ...
        threshold(fgMask, fgMask, 80, 255, THRESH_BINARY);

        // Convert fgMask to BGR for colored drawing
        Mat fgMaskColor;
        cv::cvtColor(fgMask, fgMaskColor, COLOR_GRAY2BGR);
        ...
        for (const auto& contour : contours) {
            ...
            if (area > MIN_AREA && area < MAX_AREA) {
                ...
                double intensitySum = sum(fgMask & contourMask)[0];
                ...
                double score = intensitySum - DIST_WEIGHT * dist;
                if (score > bestScore) {
                    bestScore = score;
        ...
        Mat mirroredFrame;
        cv::flip(fgMaskColor, mirroredFrame, 1);
